       ...
       {/* Dynamic light elements in the background */} 
        ...
        {!hasSubmitted ? (
          <div className="flex-grow flex flex-col items-center justify-center w-full max-w-3xl">
            <h1 id="agentDynamicGreet" className="text-4xl font-bold mb-8">AI Agent & RAG System</h1>
            <p className="text-gray-400 text-lg md:text-xl min-h-[3.5rem]">
              {greetingText}
            ...
            {messages.map((message, index) => (
              ...
              >
                {message.sender === "user" ? (
                  ...
                  <div className="bg-gray-800 border border-gray-700 text-gray-200 px-5 py-4 rounded-2xl rounded-bl-none w-full shadow-lg">
        ...
        <form 
          ...
          className="bg-gray-900/90 backdrop-blur-md border border-gray-800 px-4 py-2.5 rounded-full w-full max-w-2xl shadow-2xl flex items-center gap-3 transition-all duration-300 z-20"
        >
          <button
            type="button"
            onClick={handleAttachFile}
            title="Attach files here"
            className="text-gray-400 hover:text-white hover:bg-gray-800 p-2 rounded-full transition-colors focus:outline-none"
          >
          ...
          <input 
            ...
            className="flex-grow bg-transparent text-gray-200 px-2 py-1.5 focus:outline-none placeholder-gray-500 md:text-base disabled:opacity-50 disabled:cursor-not-allowed border-none"
          />
          <button
            type="submit"
            title="Send message to the agent"
            disabled={isGenerating || !inputValue.trim()}
            className="bg-blue-600 hover:bg-blue-700 text-white p-2.5 rounded-full font-medium transition-all disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center"
          >
