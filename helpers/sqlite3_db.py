...
class Sqlite3_Db:
    ...
    def init_db(self):
        ...
        cursor.execute(f"""
            CREATE TABLE IF NOT EXISTS {self.messages_table} (
               ...
               FOREIGN KEY (cookie) REFERENCES {self.users_table}(cookie)
            )
        """)
        conn.commit()
        conn.close()
    ...
    def is_cookie_verified(self, cookie: str) -> bool:
        conn = sqlite3.connect(self.sqlite_file_path)
        cursor = conn.cursor()
        cursor.execute(f"SELECT email FROM {self.users_table} WHERE cookie = ?", (cookie,))
    ...
    def save_verified_cookie(self, email: str, cookie: str):
        conn = sqlite3.connect(self.sqlite_file_path)
        cursor = conn.cursor()
        cursor.execute(f"INSERT OR REPLACE INTO {self.users_table} (email, cookie) VALUES (?, ?)", (email, cookie))
        conn.commit()
    ...
    def add_message(self, cookie: str, sender: str, text: str, file_name: str = None):
        ...
        cursor = conn.cursor()
        cursor.execute(f"""
           INSERT INTO {self.messages_table} (cookie, sender, text, file_name)
           VALUES (?, ?, ?, ?)
        """, (cookie, sender, text, file_name))
        conn.commit()
        conn.close()
    ...
    def get_requests_left(self, cookie: str) -> int:
        ...
        cursor = conn.cursor()
        cursor.execute(f"""
            SELECT requests_left FROM {self.users_table} WHERE cookie = ?
        """, (cookie,))
        row = cursor.fetchone()
        conn.close()
        if row:
